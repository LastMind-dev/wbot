...
exports.LoadUtils = () => {
    ...
    window.WWebJS.sendSeen = async (chatId) => {
        try {
            ...
            // Ignorar erro de markedUnread - não impede o envio
            console.warn('[WWebJS] sendSeen error (ignored):', err.message);
            return false;
        }
    };
    ...
    window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
        ...
        if (chat.groupMetadata) {
            ...
            // Converter LIDs para números de telefone nos participantes
            if (chat.groupMetadata.participants?._models) {
                chat.groupMetadata.participants._models
                    .filter(x => x.id?._serialized?.endsWith('@lid'))
                    .forEach(x => {
                        // Tentar obter o número de telefone do contato ou via LidUtils
                        if (x.contact?.phoneNumber) {
                            x.id = x.contact.phoneNumber;
                        } else if (x.id) {
                            try {
                                const phoneNumber = window.Store.LidUtils.getPhoneNumber(x.id);
                                if (phoneNumber) {
                                    x.id = phoneNumber;
                                }
                            } catch (e) {
                                // Manter o LID original se não conseguir converter
                                console.warn('Could not convert LID to phone number:', x.id?._serialized);
                            }
                        }
                    });
            }
    ...
    window.WWebJS.getAddParticipantsRpcResult = async (groupWid, participantWid) => {
        ...
        // MÉTODO 1: Tentar usar addParticipants diretamente (mais confiável para LID)
        try {
            if (window.Store.GroupParticipants.addParticipants) {
                console.log('Trying direct addParticipants method...');
                const result = await window.Store.GroupParticipants.addParticipants(group, [participantWid]);
                console.log('Direct method result:', result);
                
                // Se chegou aqui sem erro, foi sucesso
                data.code = 200;
                data.name = 'DirectMethodSuccess';
                return data;
            }
        } catch (directErr) {
            console.log('Direct method failed:', directErr.message);
            // Se o erro contém "Lid is missing", tentar método alternativo
            if (directErr.message && directErr.message.includes('Lid is missing')) {
                console.log('LID error detected, trying alternative approach...');
            }
        }
        
        // MÉTODO 2: Tentar via RPC com tratamento especial para LID
        const iqTo = window.Store.WidToJid.widToGroupJid(groupWid);
        const groupMetadata = group?.groupMetadata;
        
        // Preparar argumentos do participante
        let participantArgs;
        let phoneWid = participantWid;
        
        // Garantir que temos o número de telefone, não o LID
        if (participantWid.server === 'lid') {
            const pn = window.Store.LidUtils.getPhoneNumber(participantWid);
            if (pn) phoneWid = pn;
        }
        
        // Para grupos com LID addressing mode
        if (groupMetadata?.isLidAddressingMode) {
            let lidWid = window.Store.LidUtils.getCurrentLid(phoneWid);
            
            participantArgs = [{
                participantJid: window.Store.WidToJid.widToUserJid(phoneWid),
                ...(lidWid ? { 
                    phoneNumber: phoneWid,
                    lid: lidWid 
                } : {})
            }];
        } else {
        ...
        try {
            ...
            const addParticipant = rpcResult?.value?.addParticipant?.[0];
            if (addParticipant) {
                resultArgs = addParticipant
                ...
                if (!resultArgs) {
                    resultArgs = addParticipant.addParticipantsParticipantMixins ||
                        addParticipant.value?.addParticipantsParticipantMixins;
                }
            }
        } catch (err) {
            console.error('Error in sendAddParticipantsRPC:', err);
            
            // Se o erro é de LID, tentar método 3
            if (err.message && err.message.includes('Lid is missing')) {
                // MÉTODO 3: Tentar criar o contato primeiro e depois adicionar
                try {
                    console.log('Trying to add contact first...');
                    window.Store.Contact.gadd(phoneWid, { silent: true });
                    
                    // Aguardar um pouco para o contato ser processado
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    
                    // Tentar adicionar novamente
                    const retryResult = await window.Store.GroupParticipants.addParticipants(group, [phoneWid]);
                    console.log('Retry result:', retryResult);
                    
                    data.code = 200;
                    data.name = 'RetrySuccess';
                    return data;
                } catch (retryErr) {
        ...
        if (rpcResult.name === 'AddParticipantsResponseSuccess') {
            const code = resultArgs?.value?.error || resultArgs?.error || '200';
            data.name = resultArgs?.name;
            data.code = +code;
            data.inviteV4Code = resultArgs?.value?.addRequestCode || resultArgs?.addRequestCode;
            data.inviteV4CodeExp = (resultArgs?.value?.addRequestExpiration || resultArgs?.addRequestExpiration)?.toString();
        }
        else if (rpcResult.name === 'AddParticipantsResponseClientError') {
            const errorValue = rpcResult.value?.errorAddParticipantsClientErrors?.value;
            data.code = +(errorValue?.code || 400);
        }
        else if (rpcResult.name === 'AddParticipantsResponseServerError') {
            const errorValue = rpcResult.value?.errorServerErrors?.value;
            data.code = +(errorValue?.code || 500);
        }
    ...
    window.WWebJS.enforceLidAndPnRetrieval = async (userId) => {
        ...
        try {
            lid = isLid ? wid : window.Store.LidUtils.getCurrentLid(wid);
            phone = isLid ? window.Store.LidUtils.getPhoneNumber(wid) : wid;
        } catch (e) {
            console.error('Error in initial LID/phone retrieval:', e);
        }
        ...
        if (!isLid && !lid) {
            try {
                ...
                if (!queryResult?.wid) {
                    // Retornar pelo menos o phone se não conseguir o LID
                    return { lid: null, phone: wid };
                }
                lid = window.Store.LidUtils.getCurrentLid(wid);
            } catch (e) {
                console.error('Error querying for LID:', e);
                return { lid: null, phone: wid };
            }
        }
        ...
        if (isLid && !phone) {
            try {
                const queryResult = await window.Store.QueryExist(wid);
                if (!queryResult?.wid) {
                    // Retornar pelo menos o LID se não conseguir o phone
                    return { lid: wid, phone: null };
                }
